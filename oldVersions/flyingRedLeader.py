...
def runLeg(start_lat, start_lng, stop_lat, stop_lng, number_of_steps):
	...
	for index in indexes:
		...
		print("Red Leader [%s, %s]" % (each_lat, each_lng))
		lat = each_lat 
		lng = each_lng
		#mydatabase.put("/location/", 'testuser', {"lat": lat, "lng": lng})
		mydatabase.put("/GeoFire/Red Leader", 'l', {"0": lat, "1": lng})
		sleep(.01)

if __name__ == "__main__":

	
	intial_lat = mydatabase.get("/GeoFire/Red Leader/l", "0")
	intial_lng = mydatabase.get("/GeoFire/Red Leader/l", "1")

	lat_a, lng_a = (intial_lat, intial_lng)
	
	lat_b, lng_b = (lat_a-0.0010, lng_a-0.0013)
	
	lat_c, lng_c = (lat_b-0.0005, lng_b + 0.0001)

	lat_d, lng_d = (lat_c+ 0.0007, lng_c-0.0016)
...
try:
	...
	mydatabase.put("/GeoFire/Red Leader", 'l', {"0": intial_lat, "1": intial_lng})
