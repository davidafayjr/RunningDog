...
def runLeg(start_lat, start_lng, stop_lat, stop_lng, number_of_steps):
	...
	for index in indexes:
		...
		print("Red six [%s, %s]" % (each_lat, each_lng))
		lat = each_lat 
		lng = each_lng
		#mydatabase.put("/location/", 'testuser', {"lat": lat, "lng": lng})
		mydatabase.put("/GeoFire/Red six", 'l', {"0": lat, "1": lng})
		sleep(.01)

if __name__ == "__main__":

	
	intial_lat = mydatabase.get("/GeoFire/Red six/l", "0")
	intial_lng = mydatabase.get("/GeoFire/Red six/l", "1")

	lat_a, lng_a = (intial_lat, intial_lng)

	lat_b, lng_b = (lat_a-0.0002, lng_a - 0.0005)
	
	lat_c, lng_c = (lat_b-0.0008, lng_b + 0.0005)
